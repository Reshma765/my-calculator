/**
 * Module 2 of 4: engine.js
 * Handles core arithmetic calculations, scientific processing, and programmer logic
 */
...
export function handleInputNode(val, el, updateUI) {
    ...
    if (state.currentMode === 'programmer') {
        if (state.programmerBase === 2 && !/[01]/.test(val)) return;
...
export function executeCalculation(el, updateUI, pushLedgerHistoryNode) {
    ...
    try {
        if (state.currentMode === 'scientific') {
            ...
            if (state.programmerBase !== 10) {
                throw new Error("Requires decimal processing.");
            }
